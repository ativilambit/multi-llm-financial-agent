...
- **Sessions:** Each element has `date` (**YYYY-MM-DD**), `label` (short string), optional integer `N` (ignored for math; recomputed from earnings date + BMO/AMC anchor), `one_sigma_half_width_pct` (strictly positive; **±% half-width** as shown on the 1σ line — % of anchor, **not** full width), and `three_sigma_half_width_pct` (same units). Optional **`prob_up_pct`** (0–100): when any session includes it, the root object must also include **`daily_drift_pct`**, **`drift_source`** (allowed enum), **`drift_source_value`**, and **`drift_citation`** so bounded-drift `Φ(μN/σ)` is auditable; otherwise parsing fails.
