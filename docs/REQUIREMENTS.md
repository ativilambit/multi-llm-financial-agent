...
- **Advisory qual:quant (optional provider depth):** Providers may emit **### Qualitative deep-dive & suggested blend (advisory)** — ordered qualitative drivers and **per-row** suggested `qual : quant` integers (sum 100, qual first) with short reasoning vs cited quant signals; any pair that **differs** from the canonical horizon row **must** be labeled **`advisory`**, and the **canonical fenced blend table** must still appear **verbatim** in section 8 (no silent substitution; preserves `horizon_blend_ratio_followups`). The synthesizer merges multiple providers’ advisory grids per `synthesizer_system.md` (majority vs outlier; literals unchanged).
