...
@pytest.mark.asyncio
async def test_best_effort_upsert_skips_before_db_when_run_profile_dev(
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
    tmp_path: Path,
) -> None:
    async def _should_not_run(*_a: object, **_kw: object) -> bool:
        raise AssertionError("is_db_available should not run when run_profile is dev")
    ...
    assert any("DB write skipped: run_profile=dev (not production)" in r.message for r in caplog.records)
