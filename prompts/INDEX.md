...
| `equity_analyst.j2` | Role + injection (Jinja2 user template) | `equity_analyst/prompting.py::render_prompt` (`Orchestrator`, `cli` default `prompts/equity_analyst.j2`) | Provider fan-out markdown; `prompt_export` run artifacts |
...
| `synthesizer_system.md` | Role (synthesis instructions) | `prompt_parts:_load_prompt_file("synthesizer_system.md")` (`equity_analyst/synthesizer.py` `SYNTHESIS_SYSTEM_PROMPT`) | Synthesizer system prompt (after prepend + T0 literal injection) |
