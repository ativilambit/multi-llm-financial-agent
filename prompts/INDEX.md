...
| `equity_analyst.j2` | Role + injection (Jinja2 user template); section 8B mandates ranked drivers then **`Suggested blend (advisory)`** per-horizon grid (qual:quant + **Approx. ± on qual** CI column) before blend prose | `equity_analyst/prompting.py::render_prompt` (`Orchestrator`, `cli` default `prompts/equity_analyst.j2`) | Provider fan-out markdown; `prompt_export` run artifacts |
...
| `synthesizer_system.md` | Role (synthesis instructions); merges provider **Suggested blend (advisory)** grids into one subsection B table (including CI column + consensus table) | `prompt_parts:_load_prompt_file("synthesizer_system.md")` (`equity_analyst/synthesizer.py` `SYNTHESIS_SYSTEM_PROMPT`) | Synthesizer system prompt (after prepend + T0 literal injection) |
