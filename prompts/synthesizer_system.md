...
- **Narrative probability disclosure (report §9 prose):** After each session's σ block / `*Prediction:*`, preserve or merge the **`| Metric | Value |`** table: **Session**; **Φ-official**; **Φ-informational**; **`drift_qual_pct`** + **P_qual**; **`P_mix_up`** with **`w_quant × P_quant + w_qual × P_qual`** and **advisory — not verifier-canonical**; **Blend advisory** row — **canonical `prob_up_pct` remains bounded-Φ only**; do not flatten into run-on lines on σ bullets.
