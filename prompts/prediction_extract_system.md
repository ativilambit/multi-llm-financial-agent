...
3. **Do not** use for **`probability_up`**: **`P_mix_up`**, **`P_qual (advisory Φ)`**, **`Unbounded P(up)`**, **`LLM-derived P(up)`**, or any value labeled **advisory**, **not verifier-canonical**, **LLM-only**, or **informational only**. If only those appear for a horizon, set **`probability_up`** to **null** and mention it in **`notes`**.
