...
- **Prompts:** Expanded section 8 **Horizon & blend application** plus new **Suggested dynamic blend (advisory vs canonical)** in `equity_analyst.j2` and `synthesizer_system.md` (canonical literals unchanged; ties to advisory deep-dive). (`docs(prompts): expand horizon blend + advisory dynamic blend`)
...
- **Drive upload** Added `run_environment` (`production` | `test`, default `production`) with `RUN_ENVIRONMENT` env override. CLI **`--env`** (aliases **`--environment`**, **`--drive-env`**) sets both `env` and `run_environment`. Uploads resolve or create lowercase **`prod`** or **`test`** child folders under `drive_root_folder_id` before creating the per-run folder; `run.json` records `run_environment`, `drive_upload_parent_folder_id`, and `drive_upload_parent_folder_name`. (`feat(drive): route uploads to prod/test subfolders by run environment`)
